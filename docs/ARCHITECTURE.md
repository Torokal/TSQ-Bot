...
| `ToroSquad.Modules.Live` | TSQ Live: yayıncı oturumu durum makinesi (Twitch + Kick), resmî API uzlaştırması (`ILiveStatusProvider`), tek işlemde durum + outbox, planlayıcı, `/live-admin doctor` ([live/TSQ_LIVE.md](live/TSQ_LIVE.md)) | Diğer modüller; `Domain/Providers/Application/Persistence` Discord SDK kullanamaz; `Commands` sağlayıcıya/HTTP'ye erişemez; yalnızca planlayıcı outbox'a yazar; @everyone yalnızca `LiveCardRenderer` (`LiveArchitectureTests`) |
