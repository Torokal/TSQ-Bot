...
- Belirsiz sonuç (timeout, create sırasında 500/502/504): kör tekrar yok. Embed footer'daki `ref <marker>` ile son 50
  bot mesajında sınırlı uzlaştırma: bulunursa Sent, **doğrulanmış yoklukta** tek yeniden gönderim, uzlaştırma mümkün
...
- Düzeltmeler aynı mesajı **ping'siz** düzenler; silinmiş mesaj yeni mesajla değiştirilmez.
- Planlayıcı/dispatcher çakışması: satır başına iyimser eşzamanlılık belirteci (`Version`), çakışmada yeniden yükle+uygula.
