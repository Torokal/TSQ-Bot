...
Secret'lar yalnızca şu iki yoldan okunur (ToroSquad `.env` dosyası **okumaz**):
...
## 4. Discord uygulaması (sizin hesabınızda — bu adımları ToroSquad sizin yerinize yapmaz)
...
dotnet user-secrets set "Bot:AllowMissingSourceUrlForPrivateTesting" "true" --project src\ToroSquad.Bot   # yalnızca özel test
...
= `ToroSquadBot/0.1 (<sizin URL'niz>; <iletişim e-postanız>)`. Doctor ve başlangıç doğrulaması eksikleri söyler.

## 7. Herkese açık kullanım (production)

`Bot:SourceUrl` zorunludur (AGPL-3.0): `.\scripts\Export-Source.ps1` ile commit'in arşivini üretip yayımlayın veya herkese
açık repo commit bağlantısını verin. `Bot:OperatorContact` ayarlayın. Global komut kaydı `Discord:AllowGlobalCommandSync=true`
